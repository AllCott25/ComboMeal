    ...
    // Case 2: Both vessels are completed combinations (green vessels)
      ...
      // If not the final combo, check if they share the same parent
        ...
        if (parentCombo) {
          ...
          if (siblingsCount === 2) {
            ...
            if (hasAllRequired) {
              console.log(`These vessels fulfill all requirements for: ${parentCombo.name}`);
              resultingCombo = parentCombo;
              isValidCombination = true;
            } else {
              // We don't have all requirements yet, but we can create a yellow vessel
              console.log("These vessels share a parent but don't fulfill all requirements yet");
              // We'll create a yellow vessel later
            }
          }
        }
      }
      ...
      if (isValidCombination) {
        ...
        if (resultingCombo) {
          ...
          console.log(`Created new green vessel: ${new_v.name}`);
        } else {
          // This should not happen with our current logic, but just in case
          console.log("Created yellow vessel for partial combination");
        }
